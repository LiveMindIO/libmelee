...
(neutral) through `1.0` (the unit circle in centered request space); omitting it
preserves unit-magnitude behavior. The helper computes centered radial
components with sine and cosine, then maps each component independently into the
`[0, 1]` axes accepted by `Controller.tilt_analog`. For example, centered
components `(0.8, 0.6)` become public request coordinates `(0.9, 0.8)`.

These are controller requests, not predicted emulator, game, hardware, or
physical-gate outputs. `Controller.tilt_analog` applies libmelee's existing
per-axis `fix_analog_stick` correction afterward when enabled. Exact downstream
output is outside `stick_coordinates`' contract; the helper performs no gate
calibration or downstream-processing emulation.
