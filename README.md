...
`FrameData` is deprecated in favor of the read-only `DiscFrameData` API, which reads fighter data directly from a legally supplied NTSC 1.02 Melee ISO without extracting files. Use `DiscFrameData.action_for_state(character, action)` for runtime action IDs. Existing timing-query callers can migrate incrementally with `FrameData(iso_path="/path/to/melee.iso")`, or set `MELEE_ISO_PATH` for callers that omit `iso_path`. The same environment setting selects ISO-derived `get_framedata()` summaries and segments; unavailable runtime-only fields are `None`, while `get_raw_framedata_csv()` remains an explicitly historical CSV API. Geometry-dependent compatibility methods remain unavailable in ISO mode while runtime dynamics, articles, and locomotion are not represented.
