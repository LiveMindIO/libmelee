...
`FrameData` is deprecated in favor of the read-only `DiscFrameData` API, which reads fighter data directly from a legally supplied NTSC 1.02 Melee ISO without extracting files. Use `DiscFrameData.action_for_state(character, action)` for runtime action IDs and `animation_root_frame(...)` for nominal, unit-rate TransN translation and adjacent-sample deltas before gameplay callbacks. Existing timing-query callers can migrate incrementally with `FrameData(iso_path="/path/to/melee.iso")`, or set `MELEE_ISO_PATH` for callers that omit `iso_path`. The same environment setting selects ISO-derived `get_framedata()` summaries and segments; unavailable runtime-only fields are `None`, while `get_raw_framedata_csv()` remains an explicitly historical CSV API. Standard forward/backward `roll_end_position()` is ISO-backed; geometry-dependent compatibility methods, techs, and ledge rolls remain unavailable while runtime dynamics, articles, and collision are not represented.
